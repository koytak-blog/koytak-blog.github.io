...
「スレッドの起動方法」というタイトルにしおておりますが、ここでは、スレッドを作成するシンタックスを説明したいのではなく、バックグラウンドのスレッドにタスクを依頼する時に、どういうアーキテクチャがあるかというのを話題にしたいと思います。
大きく次の２パターンがあります。
...
いわゆる「スレッドプール」と呼ばれるのアーキテクチャです。最近のスレッドライブラリは、背後でスレッドプールを使用している場合でもそれをカプセル化しており、使用者側からはほとんど意識しなくても使えるようになっているものが多いです。
...
プログラマからすると、意識しなくて済むというのは便利ですが、そのメリットや使用すべき理由も見失ってしまいがちなので注意が必要です。
スレッドプールについてはサーバーアプリ編で詳しく取り上げます。
