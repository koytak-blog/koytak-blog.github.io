...
非同期処理を理解する上では、最低限、以下の４点について考慮する必要があります。
...
スレッドの起動方法は、使用する言語やライブラリによってシンタックスにはかなりバリエーションがあります。C# でいうなら、Thread クラスを用いるのか、TAP を使うのか、あるいは async/await を使うのかなどです。
しかし、シンタックスの話はまた別の機会に譲るとして、今、ここで議論しておきたいのは、以下の２パターンのアーキテクチャの違いです。

- 新規にスレッドを作成する
- 事前にスレッドを立ち上げておく（スレッドプールの利用）
