...
この問題の解決策は、時間のかかる処理をバックグラウンドスレッドで実行(非同期実行)させる方法です。
...
しかし、便利な方法に最初から頼るのではなく、ここでは、もうちょっと基本に立ち返って順に学習しようと思います。
まずは、以下の順番で考察します。
...
1. スレッドの取消方法（バックグラウンドスレッドの継続と途中でキャンセルするには）

どの言語でどのライブラリを使用してプログラミングした場合でも、上の挙げた処理は多くの場合、なんらかの方法で実装されている思います。

## スレッドの起動方法

スレッドの起動方法は、使用する言語やライブラリによってそのシンタックスにはかなりバリエーションがあります。C# でいうなら、Thread クラスを用いるのか、TAP を使うのか、あるいは async/await を使うのかなどです。
しかし、どのシンタックスを使用したとしても、内部的には大きく以下の２パターンが存在します。

- 新規にスレッドを作成して、タスクを実行する<br/>
説明するまでもなく、非同期処理として実行したいタスクの為に、新規にスレッドを生成する方法です。

- 事前にスレッドを立ち上げておき、タスクをそのスレッドに渡す<br/>
「スレッドプール」とか「プロデューサー／コンシューマーパターン」と呼ばれるのアーキテクチャのことです。スレッドプールについてはサーバーアプリ編で取り上げる予定なので、ここでは詳細は割愛します。

この記事で扱っている対話型アプリの場合には、スレッドを新規に起こそうがスレッドプールを利用しようが、どちらでも違いはないのでわかりづらいかもしれません。しかし、サーバーアプリのように高スループットでスレッドを生成する必要がある場合には、スレッドプールのアーキテクチャを選択する必要があります。
この為、状況によってはこの２パターンの使い分けは重要です。
